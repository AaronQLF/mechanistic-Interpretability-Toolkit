...
import { quantProbabilityChapters } from "@/lib/quant";

export default function QuantChapter() {
  return (
    <ChapterShell
      moduleSlug={`quant/probability`}
      chapterSlug="capstone-gf-gamblers-ruin"
      chapters={quantProbabilityChapters}
      eyebrow={"Probability for Interviews · 11"}
      title={"Capstone: generating functions & gambler's ruin"}
      lede={"Solve ruin probabilities and practice chain-style reasoning under time pressure."}
    >
      <h2>What interviewers want here</h2>
      <p>
        You should be able to explain <strong>{"Capstone: generating functions & gambler's ruin"}</strong> in plain language,
      ...
      <h2>Core ideas</h2>
      <p>{"Solve ruin probabilities and practice chain-style reasoning under time pressure. This chapter stays deliberately dense: skim once for the map, then work the challenge cold before you peek at the solution."}</p>
      ...
      <CodeBlock
        ...
        code={`// capstone gf gamblers ruin — adapt types and invariants
#include <cstdint>
#include <vector>
...
`}
      ...
      <Challenge
        ...
        prompt={
          <>
            <p>
              <strong>Problem {"Capstone: generating functions & gambler's ruin"}</strong> Pick one concrete sub-problem
