...
export default function QuantChapter() {
  return (
    <ChapterShell
      ...
      chapterSlug="martingales-optional-stopping"
      chapters={quantProbabilityChapters}
      eyebrow={"Probability for Interviews · 10"}
      title={"Martingales & optional stopping"}
      lede={"Fair games, Wald's equation sketch, and when to stop betting."}
    >
      <h2>What interviewers want here</h2>
      <p>
        You should be able to explain <strong>{"Martingales & optional stopping"}</strong> in plain language,
      ...
      <h2>Core ideas</h2>
      <p>{"Fair games, Wald's equation sketch, and when to stop betting. This chapter stays deliberately dense: skim once for the map, then work the challenge cold before you peek at the solution."}</p>
      ...
      <CodeBlock
        ...
        code={`// martingales optional stopping — adapt types and invariants
#include <cstdint>
#include <vector>
...
`}
      ...
      <Challenge
        ...
        prompt={
          <>
            <p>
              <strong>Problem {"Martingales & optional stopping"}</strong> Pick one concrete sub-problem
