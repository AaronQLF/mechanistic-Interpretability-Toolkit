import { MarkovChainExplorer } from "@/components/viz/quant/MarkovChainExplorer";
...
export default function QuantChapter() {
  return (
    <ChapterShell
      ...
      chapterSlug="markov-chains-basics"
      chapters={quantProbabilityChapters}
      eyebrow={"Probability for Interviews · 09"}
      title={"Markov chains"}
      lede={"Transition matrices, stationary distributions, and hitting times."}
    >
      <h2>What interviewers want here</h2>
      <p>
        You should be able to explain <strong>{"Markov chains"}</strong> in plain language,
      ...
      <h2>Core ideas</h2>
      <p>{"Transition matrices, stationary distributions, and hitting times. This chapter stays deliberately dense: skim once for the map, then work the challenge cold before you peek at the solution."}</p>
      ...
      <CodeBlock
        ...
        code={`// markov chains basics — adapt types and invariants
#include <cstdint>
#include <vector>
...
`}
      ...
      <Challenge
        ...
        prompt={
          <>
            <p>
              <strong>Problem {"Markov chains"}</strong> Pick one concrete sub-problem
