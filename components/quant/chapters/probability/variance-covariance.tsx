...
export default function QuantChapter() {
  return (
    <ChapterShell
      ...
      chapterSlug="variance-covariance"
      chapters={quantProbabilityChapters}
      eyebrow={"Probability for Interviews · 05"}
      title={"Variance, covariance & correlation"}
      lede={"Var of sums, correlation pitfalls, and portfolio intuition."}
    >
      <h2>What interviewers want here</h2>
      <p>
        You should be able to explain <strong>{"Variance, covariance & correlation"}</strong> in plain language,
      ...
      <h2>Core ideas</h2>
      <p>{"Var of sums, correlation pitfalls, and portfolio intuition. This chapter stays deliberately dense: skim once for the map, then work the challenge cold before you peek at the solution."}</p>
      ...
      <CodeBlock
        ...
        code={`// variance covariance — adapt types and invariants
#include <cstdint>
#include <vector>
...
`}
      ...
      <Challenge
        ...
        prompt={
          <>
            <p>
              <strong>Problem {"Variance, covariance & correlation"}</strong> Pick one concrete sub-problem
