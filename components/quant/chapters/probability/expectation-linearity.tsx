...
export default function QuantChapter() {
  return (
    <ChapterShell
      ...
      chapterSlug="expectation-linearity"
      chapters={quantProbabilityChapters}
      eyebrow={"Probability for Interviews · 04"}
      title={"Expectation & linearity"}
      lede={"E[XY] vs E[X]E[Y]; indicator variables and the linearity hammer."}
    >
      <h2>What interviewers want here</h2>
      <p>
        You should be able to explain <strong>{"Expectation & linearity"}</strong> in plain language,
      ...
      <h2>Core ideas</h2>
      <p>{"E[XY] vs E[X]E[Y]; indicator variables and the linearity hammer. This chapter stays deliberately dense: skim once for the map, then work the challenge cold before you peek at the solution."}</p>
      ...
      <CodeBlock
        ...
        code={`// expectation linearity — adapt types and invariants
#include <cstdint>
#include <vector>
...
`}
      ...
      <Challenge
        ...
        prompt={
          <>
            <p>
              <strong>Problem {"Expectation & linearity"}</strong> Pick one concrete sub-problem
