...
export default function QuantChapter() {
  return (
    <ChapterShell
      ...
      chapterSlug="bayes-law"
      chapters={quantProbabilityChapters}
      eyebrow={"Probability for Interviews · 03"}
      title={"Bayes' law & belief updating"}
      lede={"Posterior odds, base rate neglect, and the discrete Bayes interview pattern."}
    >
      <h2>What interviewers want here</h2>
      <p>
        You should be able to explain <strong>{"Bayes' law & belief updating"}</strong> in plain language,
      ...
      <h2>Core ideas</h2>
      <p>{"Posterior odds, base rate neglect, and the discrete Bayes interview pattern. This chapter stays deliberately dense: skim once for the map, then work the challenge cold before you peek at the solution."}</p>
      ...
      <CodeBlock
        ...
        code={`// bayes law — adapt types and invariants
#include <cstdint>
#include <vector>
...
`}
      ...
      <Challenge
        ...
        prompt={
          <>
            <p>
              <strong>Problem {"Bayes' law & belief updating"}</strong> Pick one concrete sub-problem
