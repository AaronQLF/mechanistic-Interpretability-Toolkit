...
export default function QuantChapter() {
  return (
    <ChapterShell
      ...
      chapterSlug="axioms-conditional"
      chapters={quantProbabilityChapters}
      eyebrow={"Probability for Interviews · 02"}
      title={"Axioms & conditional probability"}
      lede={"Kolmogorov axioms, independence, and the product rule — the grammar of all later chapters."}
    >
      <h2>What interviewers want here</h2>
      <p>
        You should be able to explain <strong>{"Axioms & conditional probability"}</strong> in plain language,
      ...
      <h2>Core ideas</h2>
      <p>{"Kolmogorov axioms, independence, and the product rule — the grammar of all later chapters. This chapter stays deliberately dense: skim once for the map, then work the challenge cold before you peek at the solution."}</p>
      ...
      <CodeBlock
        ...
        code={`// axioms conditional — adapt types and invariants
#include <cstdint>
#include <vector>
...
`}
      ...
      <Challenge
        ...
        prompt={
          <>
            <p>
              <strong>Problem {"Axioms & conditional probability"}</strong> Pick one concrete sub-problem
