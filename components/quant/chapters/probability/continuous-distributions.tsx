...
export default function QuantChapter() {
  return (
    <ChapterShell
      ...
      chapterSlug="continuous-distributions"
      chapters={quantProbabilityChapters}
      eyebrow={"Probability for Interviews · 07"}
      title={"Continuous distributions"}
      lede={"Uniform, exponential, normal, lognormal — when returns are not Gaussian."}
    >
      <h2>What interviewers want here</h2>
      <p>
        You should be able to explain <strong>{"Continuous distributions"}</strong> in plain language,
      ...
      <h2>Core ideas</h2>
      <p>{"Uniform, exponential, normal, lognormal — when returns are not Gaussian. This chapter stays deliberately dense: skim once for the map, then work the challenge cold before you peek at the solution."}</p>
      ...
      <CodeBlock
        ...
        code={`// continuous distributions — adapt types and invariants
#include <cstdint>
#include <vector>
...
`}
      ...
      <Challenge
        ...
        prompt={
          <>
            <p>
              <strong>Problem {"Continuous distributions"}</strong> Pick one concrete sub-problem
