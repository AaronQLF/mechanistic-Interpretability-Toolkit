import { RandomWalkBrownian } from "@/components/viz/quant/RandomWalkBrownian";
...
export default function QuantChapter() {
  return (
    <ChapterShell
      ...
      chapterSlug="random-walks-brownian"
      chapters={stochasticChapters}
      eyebrow={"Stochastic Calculus & Numerics · 01"}
      title={"Random walks & Brownian motion"}
      lede={"Scaling limits, quadratic variation preview, and simulation intuition."}
    >
      <h2>What interviewers want here</h2>
      <p>
        You should be able to explain <strong>{"Random walks & Brownian motion"}</strong> in plain language,
      ...
      <h2>Core ideas</h2>
      <p>{"Scaling limits, quadratic variation preview, and simulation intuition. This chapter stays deliberately dense: skim once for the map, then work the challenge cold before you peek at the solution."}</p>
      ...
      <CodeBlock
        ...
        code={`// random walks brownian — adapt types and invariants
#include <cstdint>
#include <vector>
...
`}
      ...
      <Challenge
        ...
        prompt={
          <>
            <p>
              <strong>Problem {"Random walks & Brownian motion"}</strong> Pick one concrete sub-problem
