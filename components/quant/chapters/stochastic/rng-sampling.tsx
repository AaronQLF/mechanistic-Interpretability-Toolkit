...
export default function QuantChapter() {
  return (
    <ChapterShell
      ...
      chapterSlug="rng-sampling"
      chapters={stochasticChapters}
      eyebrow={"Stochastic Calculus & Numerics · 08"}
      title={"RNG & sampling methods"}
      lede={"Uniform PRNGs, inverse CDF, Box–Muller, acceptance–rejection."}
    >
      <h2>What interviewers want here</h2>
      <p>
        You should be able to explain <strong>{"RNG & sampling methods"}</strong> in plain language,
      ...
      <h2>Core ideas</h2>
      <p>{"Uniform PRNGs, inverse CDF, Box–Muller, acceptance–rejection. This chapter stays deliberately dense: skim once for the map, then work the challenge cold before you peek at the solution."}</p>
      ...
      <CodeBlock
        ...
        code={`// rng sampling — adapt types and invariants
#include <cstdint>
#include <vector>
...
`}
      ...
      <Challenge
        ...
        prompt={
          <>
            <p>
              <strong>Problem {"RNG & sampling methods"}</strong> Pick one concrete sub-problem
