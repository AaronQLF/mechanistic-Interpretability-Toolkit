...
export default function QuantChapter() {
  return (
    <ChapterShell
      ...
      chapterSlug="finite-differences-pde"
      chapters={stochasticChapters}
      eyebrow={"Stochastic Calculus & Numerics · 06"}
      title={"Finite differences for PDEs"}
      lede={"Explicit scheme stability, CFL condition, boundary conditions."}
    >
      <h2>What interviewers want here</h2>
      <p>
        You should be able to explain <strong>{"Finite differences for PDEs"}</strong> in plain language,
      ...
      <h2>Core ideas</h2>
      <p>{"Explicit scheme stability, CFL condition, boundary conditions. This chapter stays deliberately dense: skim once for the map, then work the challenge cold before you peek at the solution."}</p>
      ...
      <CodeBlock
        ...
        code={`// finite differences pde — adapt types and invariants
#include <cstdint>
#include <vector>
...
`}
      ...
      <Challenge
        ...
        prompt={
          <>
            <p>
              <strong>Problem {"Finite differences for PDEs"}</strong> Pick one concrete sub-problem
