import { MonteCarloGBM } from "@/components/viz/quant/MonteCarloGBM";
...
import { stochasticChapters } from "@/lib/quant";

export default function QuantChapter() {
  return (
    <ChapterShell
      moduleSlug={`quant/stochastic`}
      chapterSlug="monte-carlo-variance"
      chapters={stochasticChapters}
      eyebrow={"Stochastic Calculus & Numerics · 07"}
      title={"Monte Carlo & variance reduction"}
      lede={"Standard error, antithetic variates, control variates — convergence you can see."}
    >
      <h2>What interviewers want here</h2>
      <p>
        You should be able to explain <strong>{"Monte Carlo & variance reduction"}</strong> in plain language,
      ...
      <h2>Core ideas</h2>
      <p>{"Standard error, antithetic variates, control variates — convergence you can see. This chapter stays deliberately dense: skim once for the map, then work the challenge cold before you peek at the solution."}</p>
      ...
      <CodeBlock
        ...
        code={`// monte carlo variance — adapt types and invariants
#include <cstdint>
#include <vector>
...
`}
      ...
      <Challenge
        ...
        prompt={
          <>
            <p>
              <strong>Problem {"Monte Carlo & variance reduction"}</strong> Pick one concrete sub-problem
