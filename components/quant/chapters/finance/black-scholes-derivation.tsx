import { BlackScholesGreeksExplorer } from "@/components/viz/quant/BlackScholesGreeksExplorer";
...
import { financeChapters } from "@/lib/quant";

export default function QuantChapter() {
  return (
    <ChapterShell
      moduleSlug={`quant/finance`}
      chapterSlug="black-scholes-derivation"
      chapters={financeChapters}
      eyebrow={"Quantitative Finance & Options · 07"}
      title={"Black–Scholes derivation sketch"}
      lede={"Replication argument, PDE, closed form — know all three layers."}
    >
      <h2>What interviewers want here</h2>
      <p>
        You should be able to explain <strong>{"Black–Scholes derivation sketch"}</strong> in plain language,
      ...
      <h2>Core ideas</h2>
      <p>{"Replication argument, PDE, closed form — know all three layers. This chapter stays deliberately dense: skim once for the map, then work the challenge cold before you peek at the solution."}</p>
      ...
      <CodeBlock
        ...
        code={`// black scholes derivation — adapt types and invariants
#include <cstdint>
#include <vector>
...
`}
      ...
      <Challenge
        ...
        prompt={
          <>
            <p>
              <strong>Problem {"Black–Scholes derivation sketch"}</strong> Pick one concrete sub-problem
