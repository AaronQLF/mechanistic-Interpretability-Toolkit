...
export default function QuantChapter() {
  return (
    <ChapterShell
      ...
      chapterSlug="option-payoffs-put-call"
      chapters={financeChapters}
      eyebrow={"Quantitative Finance & Options · 04"}
      title={"Option payoffs & put–call parity"}
      lede={"Calls, puts, spreads, and the parity relation you derive in two minutes."}
    >
      <h2>What interviewers want here</h2>
      <p>
        You should be able to explain <strong>{"Option payoffs & put–call parity"}</strong> in plain language,
      ...
      <h2>Core ideas</h2>
      <p>{"Calls, puts, spreads, and the parity relation you derive in two minutes. This chapter stays deliberately dense: skim once for the map, then work the challenge cold before you peek at the solution."}</p>
      ...
      <CodeBlock
        ...
        code={`// option payoffs put call — adapt types and invariants
#include <cstdint>
#include <vector>
...
`}
      ...
      <Challenge
        ...
        prompt={
          <>
            <p>
              <strong>Problem {"Option payoffs & put–call parity"}</strong> Pick one concrete sub-problem
