...
export default function QuantChapter() {
  return (
    <ChapterShell
      ...
      chapterSlug="fixed-income-credit"
      chapters={financeChapters}
      eyebrow={"Quantitative Finance & Options · 11"}
      title={"Fixed income & credit basics"}
      lede={"Duration, convexity, CDS intuition — enough not to embarrass yourself."}
    >
      <h2>What interviewers want here</h2>
      <p>
        You should be able to explain <strong>{"Fixed income & credit basics"}</strong> in plain language,
      ...
      <h2>Core ideas</h2>
      <p>{"Duration, convexity, CDS intuition — enough not to embarrass yourself. This chapter stays deliberately dense: skim once for the map, then work the challenge cold before you peek at the solution."}</p>
      ...
      <CodeBlock
        ...
        code={`// fixed income credit — adapt types and invariants
#include <cstdint>
#include <vector>
...
`}
      ...
      <Challenge
        ...
        prompt={
          <>
            <p>
              <strong>Problem {"Fixed income & credit basics"}</strong> Pick one concrete sub-problem
