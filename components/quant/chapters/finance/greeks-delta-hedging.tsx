...
export default function QuantChapter() {
  return (
    <ChapterShell
      ...
      chapterSlug="greeks-delta-hedging"
      chapters={financeChapters}
      eyebrow={"Quantitative Finance & Options · 08"}
      title={"The Greeks & delta hedging"}
      lede={"Delta, gamma, vega, theta — P&L attribution and gamma scalping intuition."}
    >
      <h2>What interviewers want here</h2>
      <p>
        You should be able to explain <strong>{"The Greeks & delta hedging"}</strong> in plain language,
      ...
      <h2>Core ideas</h2>
      <p>{"Delta, gamma, vega, theta — P&L attribution and gamma scalping intuition. This chapter stays deliberately dense: skim once for the map, then work the challenge cold before you peek at the solution."}</p>
      ...
      <CodeBlock
        ...
        code={`// greeks delta hedging — adapt types and invariants
#include <cstdint>
#include <vector>
...
`}
      ...
      <Challenge
        ...
        prompt={
          <>
            <p>
              <strong>Problem {"The Greeks & delta hedging"}</strong> Pick one concrete sub-problem
