...
export default function QuantChapter() {
  return (
    <ChapterShell
      ...
      chapterSlug="multicast-trading-feeds"
      chapters={osNetworkingChapters}
      eyebrow={"OS & Networking · 07"}
      title={"Multicast & trading feeds"}
      lede={"UDP multicast, sequence gaps, recovery channels — how exchanges push ticks."}
    >
      <h2>What interviewers want here</h2>
      <p>
        You should be able to explain <strong>{"Multicast & trading feeds"}</strong> in plain language,
      ...
      <h2>Core ideas</h2>
      <p>{"UDP multicast, sequence gaps, recovery channels — how exchanges push ticks. This chapter stays deliberately dense: skim once for the map, then work the challenge cold before you peek at the solution."}</p>
      ...
      <CodeBlock
        ...
        code={`// multicast trading feeds — adapt types and invariants
#include <cstdint>
#include <vector>
...
`}
      ...
      <Challenge
        ...
        prompt={
          <>
            <p>
              <strong>Problem {"Multicast & trading feeds"}</strong> Pick one concrete sub-problem
