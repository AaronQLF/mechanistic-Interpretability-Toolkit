...
export default function QuantChapter() {
  return (
    <ChapterShell
      ...
      chapterSlug="sockets-programming"
      chapters={osNetworkingChapters}
      eyebrow={"OS & Networking · 08"}
      title={"Sockets programming"}
      lede={"bind/listen/accept, non-blocking sockets, SO_REUSEPORT — minimal echo server to production patterns."}
    >
      <h2>What interviewers want here</h2>
      <p>
        You should be able to explain <strong>{"Sockets programming"}</strong> in plain language,
      ...
      <h2>Core ideas</h2>
      <p>{"bind/listen/accept, non-blocking sockets, SO_REUSEPORT — minimal echo server to production patterns. This chapter stays deliberately dense: skim once for the map, then work the challenge cold before you peek at the solution."}</p>
      ...
      <CodeBlock
        ...
        code={`// sockets programming — adapt types and invariants
#include <cstdint>
#include <vector>
...
`}
      ...
      <Challenge
        ...
        prompt={
          <>
            <p>
              <strong>Problem {"Sockets programming"}</strong> Pick one concrete sub-problem
