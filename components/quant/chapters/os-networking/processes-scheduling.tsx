...
export default function QuantChapter() {
  return (
    <ChapterShell
      ...
      chapterSlug="processes-scheduling"
      chapters={osNetworkingChapters}
      eyebrow={"OS & Networking · 01"}
      title={"Processes, threads & scheduling"}
      lede={"PID, fork/exec model vs threads, CFS and priority — what the OS guarantees."}
    >
      <h2>What interviewers want here</h2>
      <p>
        You should be able to explain <strong>{"Processes, threads & scheduling"}</strong> in plain language,
      ...
      <h2>Core ideas</h2>
      <p>{"PID, fork/exec model vs threads, CFS and priority — what the OS guarantees. This chapter stays deliberately dense: skim once for the map, then work the challenge cold before you peek at the solution."}</p>
      ...
      <CodeBlock
        ...
        code={`// processes scheduling — adapt types and invariants
#include <cstdint>
#include <vector>
...
`}
      ...
      <Challenge
        ...
        prompt={
          <>
            <p>
              <strong>Problem {"Processes, threads & scheduling"}</strong> Pick one concrete sub-problem
