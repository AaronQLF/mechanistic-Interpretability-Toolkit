...
export default function QuantChapter() {
  return (
    <ChapterShell
      ...
      chapterSlug="io-epoll-iouring"
      chapters={osNetworkingChapters}
      eyebrow={"OS & Networking · 05"}
      title={"I/O multiplexing: epoll & io_uring"}
      lede={"Edge vs level triggered, ring buffers in the kernel — the path to millions of sockets."}
    >
      <h2>What interviewers want here</h2>
      <p>
        You should be able to explain <strong>{"I/O multiplexing: epoll & io_uring"}</strong> in plain language,
      ...
      <h2>Core ideas</h2>
      <p>{"Edge vs level triggered, ring buffers in the kernel — the path to millions of sockets. This chapter stays deliberately dense: skim once for the map, then work the challenge cold before you peek at the solution."}</p>
      ...
      <CodeBlock
        ...
        code={`// io epoll iouring — adapt types and invariants
#include <cstdint>
#include <vector>
...
`}
      ...
      <Challenge
        ...
        prompt={
          <>
            <p>
              <strong>Problem {"I/O multiplexing: epoll & io_uring"}</strong> Pick one concrete sub-problem
