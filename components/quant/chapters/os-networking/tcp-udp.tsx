...
export default function QuantChapter() {
  return (
    <ChapterShell
      ...
      eyebrow={"OS & Networking · 06"}
      title={"TCP & UDP"}
      lede={"Handshake, congestion control, head-of-line blocking — when each is right for market data."}
    >
      <h2>What interviewers want here</h2>
      <p>
        You should be able to explain <strong>{"TCP & UDP"}</strong> in plain language,
      ...
      <h2>Core ideas</h2>
      <p>{"Handshake, congestion control, head-of-line blocking — when each is right for market data. This chapter stays deliberately dense: skim once for the map, then work the challenge cold before you peek at the solution."}</p>
      ...
      <CodeBlock
        ...
        code={`// tcp udp — adapt types and invariants
#include <cstdint>
#include <vector>
...
`}
      ...
      <Challenge
        ...
        prompt={
          <>
            <p>
              <strong>Problem {"TCP & UDP"}</strong> Pick one concrete sub-problem
