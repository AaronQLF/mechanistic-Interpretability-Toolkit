...
export default function QuantChapter() {
  return (
    <ChapterShell
      ...
      chapterSlug="virtual-memory"
      chapters={osNetworkingChapters}
      eyebrow={"OS & Networking · 02"}
      title={"Virtual memory & paging"}
      lede={"Page tables, TLB, mmap — why random pointer chasing hurts."}
    >
      <h2>What interviewers want here</h2>
      <p>
        You should be able to explain <strong>{"Virtual memory & paging"}</strong> in plain language,
      ...
      <h2>Core ideas</h2>
      <p>{"Page tables, TLB, mmap — why random pointer chasing hurts. This chapter stays deliberately dense: skim once for the map, then work the challenge cold before you peek at the solution."}</p>
      ...
      <CodeBlock
        ...
        code={`// virtual memory — adapt types and invariants
#include <cstdint>
#include <vector>
...
`}
      ...
      <Challenge
        ...
        prompt={
          <>
            <p>
              <strong>Problem {"Virtual memory & paging"}</strong> Pick one concrete sub-problem
