...
export default function QuantChapter() {
  return (
    <ChapterShell
      ...
      chapterSlug="capstone-os-net-qa"
      chapters={osNetworkingChapters}
      eyebrow={"OS & Networking · 09"}
      title={"Capstone: OS & networking Q&A"}
      lede={"Whiteboard scenarios: tail latency debugging, TCP vs UDP for a new feed, epoll design."}
    >
      <h2>What interviewers want here</h2>
      <p>
        You should be able to explain <strong>{"Capstone: OS & networking Q&A"}</strong> in plain language,
      ...
      <h2>Core ideas</h2>
      <p>{"Whiteboard scenarios: tail latency debugging, TCP vs UDP for a new feed, epoll design. This chapter stays deliberately dense: skim once for the map, then work the challenge cold before you peek at the solution."}</p>
      ...
      <CodeBlock
        ...
        code={`// capstone os net qa — adapt types and invariants
#include <cstdint>
#include <vector>
...
`}
      ...
      <Challenge
        ...
        prompt={
          <>
            <p>
              <strong>Problem {"Capstone: OS & networking Q&A"}</strong> Pick one concrete sub-problem
