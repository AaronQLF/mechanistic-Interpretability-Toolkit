...
export default function QuantChapter() {
  return (
    <ChapterShell
      ...
      chapterSlug="os-sync-primitives"
      chapters={osNetworkingChapters}
      eyebrow={"OS & Networking · 04"}
      title={"OS synchronization primitives"}
      lede={"Futexes, semaphores, eventfd — how pthreads map to the kernel."}
    >
      <h2>What interviewers want here</h2>
      <p>
        You should be able to explain <strong>{"OS synchronization primitives"}</strong> in plain language,
      ...
      <h2>Core ideas</h2>
      <p>{"Futexes, semaphores, eventfd — how pthreads map to the kernel. This chapter stays deliberately dense: skim once for the map, then work the challenge cold before you peek at the solution."}</p>
      ...
      <CodeBlock
        ...
        code={`// os sync primitives — adapt types and invariants
#include <cstdint>
#include <vector>
...
`}
      ...
      <Challenge
        ...
        prompt={
          <>
            <p>
              <strong>Problem {"OS synchronization primitives"}</strong> Pick one concrete sub-problem
