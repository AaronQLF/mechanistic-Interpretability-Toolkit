import { BigOExplorer } from "@/components/viz/quant/BigOExplorer";
...
import { dsaChapters } from "@/lib/quant";

export default function QuantChapter() {
  return (
    <ChapterShell
      moduleSlug={`quant/dsa`}
      chapterSlug="complexity-amortized"
      chapters={dsaChapters}
      eyebrow={"Data Structures & Algorithms · 01"}
      title={"Complexity & amortized analysis"}
      lede={"Big-O, Ω, Θ; why vector::push_back is amortized O(1); accounting and potential methods."}
    >
      <h2>What interviewers want here</h2>
      <p>
        You should be able to explain <strong>{"Complexity & amortized analysis"}</strong> in plain language,
      ...
      <h2>Core ideas</h2>
      <p>{"Big-O, Ω, Θ; why vector::push_back is amortized O(1); accounting and potential methods. This chapter stays deliberately dense: skim once for the map, then work the challenge cold before you peek at the solution."}</p>
      ...
      <CodeBlock
        ...
        code={`// complexity amortized — adapt types and invariants
#include <cstdint>
#include <vector>
...
`}
      ...
      <Challenge
        ...
        prompt={
          <>
            <p>
              <strong>Problem {"Complexity & amortized analysis"}</strong> Pick one concrete sub-problem
