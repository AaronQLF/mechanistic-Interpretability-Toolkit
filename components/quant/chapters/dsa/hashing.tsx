...
export default function QuantChapter() {
  return (
    <ChapterShell
      ...
      eyebrow={"Data Structures & Algorithms · 03"}
      title={"Hashing"}
      lede={"Hash tables, collisions, rolling hash — when O(1) average is enough."}
    >
      <h2>What interviewers want here</h2>
      <p>
        You should be able to explain <strong>{"Hashing"}</strong> in plain language,
      ...
      <h2>Core ideas</h2>
      <p>{"Hash tables, collisions, rolling hash — when O(1) average is enough. This chapter stays deliberately dense: skim once for the map, then work the challenge cold before you peek at the solution."}</p>
      ...
      <CodeBlock
        ...
        code={`// hashing — adapt types and invariants
#include <cstdint>
#include <vector>
...
`}
      ...
      <Challenge
        ...
        prompt={
          <>
            <p>
              <strong>Problem {"Hashing"}</strong> Pick one concrete sub-problem
