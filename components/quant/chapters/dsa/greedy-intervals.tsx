...
export default function QuantChapter() {
  return (
    <ChapterShell
      ...
      chapterSlug="greedy-intervals"
      chapters={dsaChapters}
      eyebrow={"Data Structures & Algorithms · 15"}
      title={"Greedy & interval scheduling"}
      lede={"Exchange arguments and classic interval problems."}
    >
      <h2>What interviewers want here</h2>
      <p>
        You should be able to explain <strong>{"Greedy & interval scheduling"}</strong> in plain language,
      ...
      <h2>Core ideas</h2>
      <p>{"Exchange arguments and classic interval problems. This chapter stays deliberately dense: skim once for the map, then work the challenge cold before you peek at the solution."}</p>
      ...
      <CodeBlock
        ...
        code={`// greedy intervals — adapt types and invariants
#include <cstdint>
#include <vector>
...
`}
      ...
      <Challenge
        ...
        prompt={
          <>
            <p>
              <strong>Problem {"Greedy & interval scheduling"}</strong> Pick one concrete sub-problem
