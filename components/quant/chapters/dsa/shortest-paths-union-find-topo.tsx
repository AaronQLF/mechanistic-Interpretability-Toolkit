...
export default function QuantChapter() {
  return (
    <ChapterShell
      ...
      chapterSlug="shortest-paths-union-find-topo"
      chapters={dsaChapters}
      eyebrow={"Data Structures & Algorithms · 11"}
      title={"Shortest paths, union-find & advanced graphs"}
      lede={"Dijkstra, Bellman-Ford, Floyd-Warshall; DSU with path compression."}
    >
      <h2>What interviewers want here</h2>
      <p>
        You should be able to explain <strong>{"Shortest paths, union-find & advanced graphs"}</strong> in plain language,
      ...
      <h2>Core ideas</h2>
      <p>{"Dijkstra, Bellman-Ford, Floyd-Warshall; DSU with path compression. This chapter stays deliberately dense: skim once for the map, then work the challenge cold before you peek at the solution."}</p>
      ...
      <CodeBlock
        ...
        code={`// shortest paths union find topo — adapt types and invariants
#include <cstdint>
#include <vector>
...
`}
      ...
      <Challenge
        ...
        prompt={
          <>
            <p>
              <strong>Problem {"Shortest paths, union-find & advanced graphs"}</strong> Pick one concrete sub-problem
