import { SortingVisualizer } from "@/components/viz/quant/SortingVisualizer";
...
export default function QuantChapter() {
  return (
    <ChapterShell
      ...
      chapterSlug="sorting-searching"
      chapters={dsaChapters}
      eyebrow={"Data Structures & Algorithms · 12"}
      title={"Sorting & binary search"}
      lede={"Stability, lower_bound patterns, and binary search on answer."}
    >
      <h2>What interviewers want here</h2>
      <p>
        You should be able to explain <strong>{"Sorting & binary search"}</strong> in plain language,
      ...
      <h2>Core ideas</h2>
      <p>{"Stability, lower_bound patterns, and binary search on answer. This chapter stays deliberately dense: skim once for the map, then work the challenge cold before you peek at the solution."}</p>
      ...
      <CodeBlock
        ...
        code={`// sorting searching — adapt types and invariants
#include <cstdint>
#include <vector>
...
`}
      ...
      <Challenge
        ...
        prompt={
          <>
            <p>
              <strong>Problem {"Sorting & binary search"}</strong> Pick one concrete sub-problem
