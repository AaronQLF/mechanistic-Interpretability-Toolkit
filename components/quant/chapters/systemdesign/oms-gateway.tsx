...
export default function QuantChapter() {
  return (
    <ChapterShell
      ...
      chapterSlug="oms-gateway"
      chapters={systemdesignChapters}
      eyebrow={"Trading System Design · 05"}
      title={"OMS & execution gateway"}
      lede={"Client connectivity, idempotency, and state machines for orders."}
    >
      <h2>What interviewers want here</h2>
      <p>
        You should be able to explain <strong>{"OMS & execution gateway"}</strong> in plain language,
      ...
      <h2>Core ideas</h2>
      <p>{"Client connectivity, idempotency, and state machines for orders. This chapter stays deliberately dense: skim once for the map, then work the challenge cold before you peek at the solution."}</p>
      ...
      <CodeBlock
        ...
        code={`// oms gateway — adapt types and invariants
#include <cstdint>
#include <vector>
...
`}
      ...
      <Challenge
        ...
        prompt={
          <>
            <p>
              <strong>Problem {"OMS & execution gateway"}</strong> Pick one concrete sub-problem
