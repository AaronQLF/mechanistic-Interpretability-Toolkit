...
export default function QuantChapter() {
  return (
    <ChapterShell
      ...
      chapterSlug="event-driven-messaging"
      chapters={systemdesignChapters}
      eyebrow={"Trading System Design · 08"}
      title={"Event-driven messaging"}
      lede={"Pub/sub, Kafka vs Aeron, at-least-once vs exactly-once trade-offs."}
    >
      <h2>What interviewers want here</h2>
      <p>
        You should be able to explain <strong>{"Event-driven messaging"}</strong> in plain language,
      ...
      <h2>Core ideas</h2>
      <p>{"Pub/sub, Kafka vs Aeron, at-least-once vs exactly-once trade-offs. This chapter stays deliberately dense: skim once for the map, then work the challenge cold before you peek at the solution."}</p>
      ...
      <CodeBlock
        ...
        code={`// event driven messaging — adapt types and invariants
#include <cstdint>
#include <vector>
...
`}
      ...
      <Challenge
        ...
        prompt={
          <>
            <p>
              <strong>Problem {"Event-driven messaging"}</strong> Pick one concrete sub-problem
