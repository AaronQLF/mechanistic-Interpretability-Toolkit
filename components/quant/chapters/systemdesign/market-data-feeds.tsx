...
export default function QuantChapter() {
  return (
    <ChapterShell
      ...
      chapterSlug="market-data-feeds"
      chapters={systemdesignChapters}
      eyebrow={"Trading System Design · 04"}
      title={"Market data feed handlers"}
      lede={"Incremental updates, snapshots, sequence numbers, and fan-out."}
    >
      <h2>What interviewers want here</h2>
      <p>
        You should be able to explain <strong>{"Market data feed handlers"}</strong> in plain language,
      ...
      <h2>Core ideas</h2>
      <p>{"Incremental updates, snapshots, sequence numbers, and fan-out. This chapter stays deliberately dense: skim once for the map, then work the challenge cold before you peek at the solution."}</p>
      ...
      <CodeBlock
        ...
        code={`// market data feeds — adapt types and invariants
#include <cstdint>
#include <vector>
...
`}
      ...
      <Challenge
        ...
        prompt={
          <>
            <p>
              <strong>Problem {"Market data feed handlers"}</strong> Pick one concrete sub-problem
