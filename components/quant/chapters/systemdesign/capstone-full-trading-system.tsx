...
export default function QuantChapter() {
  return (
    <ChapterShell
      ...
      chapterSlug="capstone-full-trading-system"
      chapters={systemdesignChapters}
      eyebrow={"Trading System Design · 11"}
      title={"Capstone: design a full trading system"}
      lede={"End-to-end: feed → signals → risk → orders → fills → P&L — one coherent narrative."}
    >
      <h2>What interviewers want here</h2>
      <p>
        You should be able to explain <strong>{"Capstone: design a full trading system"}</strong> in plain language,
      ...
      <h2>Core ideas</h2>
      <p>{"End-to-end: feed → signals → risk → orders → fills → P&L — one coherent narrative. This chapter stays deliberately dense: skim once for the map, then work the challenge cold before you peek at the solution."}</p>
      ...
      <CodeBlock
        ...
        code={`// capstone full trading system — adapt types and invariants
#include <cstdint>
#include <vector>
...
`}
      ...
      <Challenge
        ...
        prompt={
          <>
            <p>
              <strong>Problem {"Capstone: design a full trading system"}</strong> Pick one concrete sub-problem
