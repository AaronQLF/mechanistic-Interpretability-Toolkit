...
export default function QuantChapter() {
  return (
    <ChapterShell
      ...
      chapterSlug="tick-databases"
      chapters={systemdesignChapters}
      eyebrow={"Trading System Design · 07"}
      title={"Tick databases & storage"}
      lede={"Time-series DBs, columnar formats, and replay for backtests."}
    >
      <h2>What interviewers want here</h2>
      <p>
        You should be able to explain <strong>{"Tick databases & storage"}</strong> in plain language,
      ...
      <h2>Core ideas</h2>
      <p>{"Time-series DBs, columnar formats, and replay for backtests. This chapter stays deliberately dense: skim once for the map, then work the challenge cold before you peek at the solution."}</p>
      ...
      <CodeBlock
        ...
        code={`// tick databases — adapt types and invariants
#include <cstdint>
#include <vector>
...
`}
      ...
      <Challenge
        ...
        prompt={
          <>
            <p>
              <strong>Problem {"Tick databases & storage"}</strong> Pick one concrete sub-problem
