import { OrderBookSimulator } from "@/components/viz/quant/OrderBookSimulator";
...
import { systemdesignChapters } from "@/lib/quant";

export default function QuantChapter() {
  return (
    <ChapterShell
      moduleSlug={`quant/systemdesign`}
      chapterSlug="limit-order-book"
      chapters={systemdesignChapters}
      eyebrow={"Trading System Design · 02"}
      title={"Limit order book design"}
      lede={"Price levels, FIFO queues, and the data structures that survive scrutiny."}
    >
      <h2>What interviewers want here</h2>
      <p>
        You should be able to explain <strong>{"Limit order book design"}</strong> in plain language,
      ...
      <h2>Core ideas</h2>
      <p>{"Price levels, FIFO queues, and the data structures that survive scrutiny. This chapter stays deliberately dense: skim once for the map, then work the challenge cold before you peek at the solution."}</p>
      ...
      <CodeBlock
        ...
        code={`// limit order book — adapt types and invariants
#include <cstdint>
#include <vector>
...
`}
      ...
      <Challenge
        ...
        prompt={
          <>
            <p>
              <strong>Problem {"Limit order book design"}</strong> Pick one concrete sub-problem
