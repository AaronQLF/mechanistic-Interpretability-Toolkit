...
export default function QuantChapter() {
  return (
    <ChapterShell
      ...
      chapterSlug="matching-engine"
      chapters={systemdesignChapters}
      eyebrow={"Trading System Design · 03"}
      title={"Matching engine"}
      lede={"Order types, priority, auction vs continuous matching — correctness first."}
    >
      <h2>What interviewers want here</h2>
      <p>
        You should be able to explain <strong>{"Matching engine"}</strong> in plain language,
      ...
      <h2>Core ideas</h2>
      <p>{"Order types, priority, auction vs continuous matching — correctness first. This chapter stays deliberately dense: skim once for the map, then work the challenge cold before you peek at the solution."}</p>
      ...
      <CodeBlock
        ...
        code={`// matching engine — adapt types and invariants
#include <cstdint>
#include <vector>
...
`}
      ...
      <Challenge
        ...
        prompt={
          <>
            <p>
              <strong>Problem {"Matching engine"}</strong> Pick one concrete sub-problem
