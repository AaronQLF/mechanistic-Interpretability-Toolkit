...
export default function QuantChapter() {
  return (
    <ChapterShell
      ...
      chapterSlug="pre-trade-risk"
      chapters={systemdesignChapters}
      eyebrow={"Trading System Design · 06"}
      title={"Pre-trade risk checks"}
      lede={"Limits, fat-finger checks, and kill switches — compliance as latency budget."}
    >
      <h2>What interviewers want here</h2>
      <p>
        You should be able to explain <strong>{"Pre-trade risk checks"}</strong> in plain language,
      ...
      <h2>Core ideas</h2>
      <p>{"Limits, fat-finger checks, and kill switches — compliance as latency budget. This chapter stays deliberately dense: skim once for the map, then work the challenge cold before you peek at the solution."}</p>
      ...
      <CodeBlock
        ...
        code={`// pre trade risk — adapt types and invariants
#include <cstdint>
#include <vector>
...
`}
      ...
      <Challenge
        ...
        prompt={
          <>
            <p>
              <strong>Problem {"Pre-trade risk checks"}</strong> Pick one concrete sub-problem
