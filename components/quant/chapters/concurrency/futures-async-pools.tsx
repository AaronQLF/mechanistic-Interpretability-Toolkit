...
export default function QuantChapter() {
  return (
    <ChapterShell
      ...
      chapterSlug="futures-async-pools"
      chapters={concurrencyChapters}
      eyebrow={"Concurrency & Lock-Free · 07"}
      title={"Futures, async & thread pools"}
      lede={"Task queues, work stealing at a high level, and when to avoid std::async."}
    >
      <h2>What interviewers want here</h2>
      <p>
        You should be able to explain <strong>{"Futures, async & thread pools"}</strong> in plain language,
      ...
      <h2>Core ideas</h2>
      <p>{"Task queues, work stealing at a high level, and when to avoid std::async. This chapter stays deliberately dense: skim once for the map, then work the challenge cold before you peek at the solution."}</p>
      ...
      <CodeBlock
        ...
        code={`// futures async pools — adapt types and invariants
#include <cstdint>
#include <vector>
...
`}
      ...
      <Challenge
        ...
        prompt={
          <>
            <p>
              <strong>Problem {"Futures, async & thread pools"}</strong> Pick one concrete sub-problem
