...
export default function QuantChapter() {
  return (
    <ChapterShell
      ...
      chapterSlug="deadlock-lock-ordering"
      chapters={concurrencyChapters}
      eyebrow={"Concurrency & Lock-Free · 08"}
      title={"Deadlock & lock ordering"}
      lede={"Global lock order, try_lock patterns, and timeouts."}
    >
      <h2>What interviewers want here</h2>
      <p>
        You should be able to explain <strong>{"Deadlock & lock ordering"}</strong> in plain language,
      ...
      <h2>Core ideas</h2>
      <p>{"Global lock order, try_lock patterns, and timeouts. This chapter stays deliberately dense: skim once for the map, then work the challenge cold before you peek at the solution."}</p>
      ...
      <CodeBlock
        ...
        code={`// deadlock lock ordering — adapt types and invariants
#include <cstdint>
#include <vector>
...
`}
      ...
      <Challenge
        ...
        prompt={
          <>
            <p>
              <strong>Problem {"Deadlock & lock ordering"}</strong> Pick one concrete sub-problem
