...
export default function QuantChapter() {
  return (
    <ChapterShell
      ...
      chapterSlug="memory-model-atomics"
      chapters={concurrencyChapters}
      eyebrow={"Concurrency & Lock-Free · 03"}
      title={"The C++ memory model & atomics"}
      lede={"Happens-before, memory_order_seq_cst vs acquire/release, and fences."}
    >
      <h2>What interviewers want here</h2>
      <p>
        You should be able to explain <strong>{"The C++ memory model & atomics"}</strong> in plain language,
      ...
      <h2>Core ideas</h2>
      <p>{"Happens-before, memory_order_seq_cst vs acquire/release, and fences. This chapter stays deliberately dense: skim once for the map, then work the challenge cold before you peek at the solution."}</p>
      ...
      <CodeBlock
        ...
        code={`// memory model atomics — adapt types and invariants
#include <cstdint>
#include <vector>
...
`}
      ...
      <Challenge
        ...
        prompt={
          <>
            <p>
              <strong>Problem {"The C++ memory model & atomics"}</strong> Pick one concrete sub-problem
