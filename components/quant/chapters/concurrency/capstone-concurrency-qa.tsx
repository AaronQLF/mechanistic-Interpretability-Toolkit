...
export default function QuantChapter() {
  return (
    <ChapterShell
      ...
      chapterSlug="capstone-concurrency-qa"
      chapters={concurrencyChapters}
      eyebrow={"Concurrency & Lock-Free · 09"}
      title={"Capstone: concurrency Q&A"}
      lede={"Scenarios: producer-consumer, rate limiter, concurrent LRU sketch — verbal + code."}
    >
      <h2>What interviewers want here</h2>
      <p>
        You should be able to explain <strong>{"Capstone: concurrency Q&A"}</strong> in plain language,
      ...
      <h2>Core ideas</h2>
      <p>{"Scenarios: producer-consumer, rate limiter, concurrent LRU sketch — verbal + code. This chapter stays deliberately dense: skim once for the map, then work the challenge cold before you peek at the solution."}</p>
      ...
      <CodeBlock
        ...
        code={`// capstone concurrency qa — adapt types and invariants
#include <cstdint>
#include <vector>
...
`}
      ...
      <Challenge
        ...
        prompt={
          <>
            <p>
              <strong>Problem {"Capstone: concurrency Q&A"}</strong> Pick one concrete sub-problem
