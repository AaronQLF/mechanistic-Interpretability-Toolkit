...
import { concurrencyChapters } from "@/lib/quant";

export default function QuantChapter() {
  return (
    <ChapterShell
      moduleSlug={`quant/concurrency`}
      chapterSlug="mutex-condvar"
      chapters={concurrencyChapters}
      eyebrow={"Concurrency & Lock-Free · 02"}
      title={"Mutexes, locks & condition variables"}
      lede={"std::mutex, unique_lock, scoped_lock, wait predicates — never wake spuriously wrong."}
    >
      <h2>What interviewers want here</h2>
      <p>
        You should be able to explain <strong>{"Mutexes, locks & condition variables"}</strong> in plain language,
      ...
      <h2>Core ideas</h2>
      <p>{"std::mutex, unique_lock, scoped_lock, wait predicates — never wake spuriously wrong. This chapter stays deliberately dense: skim once for the map, then work the challenge cold before you peek at the solution."}</p>
      ...
      <CodeBlock
        ...
        code={`// mutex condvar — adapt types and invariants
#include <cstdint>
#include <vector>
...
`}
      ...
      <Challenge
        ...
        prompt={
          <>
            <p>
              <strong>Problem {"Mutexes, locks & condition variables"}</strong> Pick one concrete sub-problem
