...
export default function QuantChapter() {
  return (
    <ChapterShell
      ...
      chapterSlug="false-sharing-cache"
      chapters={concurrencyChapters}
      eyebrow={"Concurrency & Lock-Free · 06"}
      title={"False sharing & cache coherence"}
      lede={"Why two counters on one cache line destroy scaling; alignment and padding."}
    >
      <h2>What interviewers want here</h2>
      <p>
        You should be able to explain <strong>{"False sharing & cache coherence"}</strong> in plain language,
      ...
      <h2>Core ideas</h2>
      <p>{"Why two counters on one cache line destroy scaling; alignment and padding. This chapter stays deliberately dense: skim once for the map, then work the challenge cold before you peek at the solution."}</p>
      ...
      <CodeBlock
        ...
        code={`// false sharing cache — adapt types and invariants
#include <cstdint>
#include <vector>
...
`}
      ...
      <Challenge
        ...
        prompt={
          <>
            <p>
              <strong>Problem {"False sharing & cache coherence"}</strong> Pick one concrete sub-problem
