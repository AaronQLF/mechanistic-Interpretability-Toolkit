...
export default function QuantChapter() {
  return (
    <ChapterShell
      ...
      chapterSlug="threads-races"
      chapters={concurrencyChapters}
      eyebrow={"Concurrency & Lock-Free · 01"}
      title={"Threads & data races"}
      lede={"std::thread, shared state, and why races are undefined behavior in C++."}
    >
      <h2>What interviewers want here</h2>
      <p>
        You should be able to explain <strong>{"Threads & data races"}</strong> in plain language,
      ...
      <h2>Core ideas</h2>
      <p>{"std::thread, shared state, and why races are undefined behavior in C++. This chapter stays deliberately dense: skim once for the map, then work the challenge cold before you peek at the solution."}</p>
      ...
      <CodeBlock
        ...
        code={`// threads races — adapt types and invariants
#include <cstdint>
#include <vector>
...
`}
      ...
      <Challenge
        ...
        prompt={
          <>
            <p>
              <strong>Problem {"Threads & data races"}</strong> Pick one concrete sub-problem
