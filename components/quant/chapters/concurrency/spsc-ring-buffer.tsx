...
export default function QuantChapter() {
  return (
    <ChapterShell
      ...
      chapterSlug="spsc-ring-buffer"
      chapters={concurrencyChapters}
      eyebrow={"Concurrency & Lock-Free · 05"}
      title={"Build an SPSC ring buffer"}
      lede={"Single-producer single-consumer queue with cache-line padding — the HFT hello world."}
    >
      <h2>What interviewers want here</h2>
      <p>
        You should be able to explain <strong>{"Build an SPSC ring buffer"}</strong> in plain language,
      ...
      <h2>Core ideas</h2>
      <p>{"Single-producer single-consumer queue with cache-line padding — the HFT hello world. This chapter stays deliberately dense: skim once for the map, then work the challenge cold before you peek at the solution."}</p>
      ...
      <CodeBlock
        ...
        code={`// spsc ring buffer — adapt types and invariants
#include <cstdint>
#include <vector>
...
`}
      ...
      <Challenge
        ...
        prompt={
          <>
            <p>
              <strong>Problem {"Build an SPSC ring buffer"}</strong> Pick one concrete sub-problem
