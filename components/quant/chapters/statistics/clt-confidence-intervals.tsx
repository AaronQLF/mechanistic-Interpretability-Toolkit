...
import { statisticsChapters } from "@/lib/quant";

export default function QuantChapter() {
  return (
    <ChapterShell
      moduleSlug={`quant/statistics`}
      chapterSlug="clt-confidence-intervals"
      chapters={statisticsChapters}
      eyebrow={"Statistics & Time Series · 02"}
      title={"CLT & confidence intervals"}
      lede={"Normal approximations, z vs t, and interpreting intervals correctly."}
    >
      <h2>What interviewers want here</h2>
      <p>
        You should be able to explain <strong>{"CLT & confidence intervals"}</strong> in plain language,
      ...
      <h2>Core ideas</h2>
      <p>{"Normal approximations, z vs t, and interpreting intervals correctly. This chapter stays deliberately dense: skim once for the map, then work the challenge cold before you peek at the solution."}</p>
      ...
      <CodeBlock
        ...
        code={`// clt confidence intervals — adapt types and invariants
#include <cstdint>
#include <vector>
...
`}
      ...
      <Challenge
        ...
        prompt={
          <>
            <p>
              <strong>Problem {"CLT & confidence intervals"}</strong> Pick one concrete sub-problem
