...
export default function QuantChapter() {
  return (
    <ChapterShell
      ...
      chapterSlug="bias-variance-regularization"
      chapters={statisticsChapters}
      eyebrow={"Statistics & Time Series · 05"}
      title={"Bias–variance & regularization"}
      lede={"Ridge, Lasso intuition, and cross-validation discipline."}
    >
      <h2>What interviewers want here</h2>
      <p>
        You should be able to explain <strong>{"Bias–variance & regularization"}</strong> in plain language,
      ...
      <h2>Core ideas</h2>
      <p>{"Ridge, Lasso intuition, and cross-validation discipline. This chapter stays deliberately dense: skim once for the map, then work the challenge cold before you peek at the solution."}</p>
      ...
      <CodeBlock
        ...
        code={`// bias variance regularization — adapt types and invariants
#include <cstdint>
#include <vector>
...
`}
      ...
      <Challenge
        ...
        prompt={
          <>
            <p>
              <strong>Problem {"Bias–variance & regularization"}</strong> Pick one concrete sub-problem
