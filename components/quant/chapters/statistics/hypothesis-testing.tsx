...
export default function QuantChapter() {
  return (
    <ChapterShell
      ...
      chapterSlug="hypothesis-testing"
      chapters={statisticsChapters}
      eyebrow={"Statistics & Time Series · 03"}
      title={"Hypothesis testing"}
      lede={"p-values, power, multiple testing — what practitioners get wrong."}
    >
      <h2>What interviewers want here</h2>
      <p>
        You should be able to explain <strong>{"Hypothesis testing"}</strong> in plain language,
      ...
      <h2>Core ideas</h2>
      <p>{"p-values, power, multiple testing — what practitioners get wrong. This chapter stays deliberately dense: skim once for the map, then work the challenge cold before you peek at the solution."}</p>
      ...
      <CodeBlock
        ...
        code={`// hypothesis testing — adapt types and invariants
#include <cstdint>
#include <vector>
...
`}
      ...
      <Challenge
        ...
        prompt={
          <>
            <p>
              <strong>Problem {"Hypothesis testing"}</strong> Pick one concrete sub-problem
