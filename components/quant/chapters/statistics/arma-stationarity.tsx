...
export default function QuantChapter() {
  return (
    <ChapterShell
      ...
      chapterSlug="arma-stationarity"
      chapters={statisticsChapters}
      eyebrow={"Statistics & Time Series · 07"}
      title={"AR / MA / ARMA & stationarity"}
      lede={"Unit roots, ACF/PACF — the time-series language of econometrics interviews."}
    >
      <h2>What interviewers want here</h2>
      <p>
        You should be able to explain <strong>{"AR / MA / ARMA & stationarity"}</strong> in plain language,
      ...
      <h2>Core ideas</h2>
      <p>{"Unit roots, ACF/PACF — the time-series language of econometrics interviews. This chapter stays deliberately dense: skim once for the map, then work the challenge cold before you peek at the solution."}</p>
      ...
      <CodeBlock
        ...
        code={`// arma stationarity — adapt types and invariants
#include <cstdint>
#include <vector>
...
`}
      ...
      <Challenge
        ...
        prompt={
          <>
            <p>
              <strong>Problem {"AR / MA / ARMA & stationarity"}</strong> Pick one concrete sub-problem
