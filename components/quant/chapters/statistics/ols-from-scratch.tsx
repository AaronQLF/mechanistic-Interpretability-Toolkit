...
export default function QuantChapter() {
  return (
    <ChapterShell
      ...
      chapterSlug="ols-from-scratch"
      chapters={statisticsChapters}
      eyebrow={"Statistics & Time Series · 04"}
      title={"OLS from scratch"}
      lede={"Normal equations, geometric view, and regularized variants."}
    >
      <h2>What interviewers want here</h2>
      <p>
        You should be able to explain <strong>{"OLS from scratch"}</strong> in plain language,
      ...
      <h2>Core ideas</h2>
      <p>{"Normal equations, geometric view, and regularized variants. This chapter stays deliberately dense: skim once for the map, then work the challenge cold before you peek at the solution."}</p>
      ...
      <CodeBlock
        ...
        code={`// ols from scratch — adapt types and invariants
#include <cstdint>
#include <vector>
...
`}
      ...
      <Challenge
        ...
        prompt={
          <>
            <p>
              <strong>Problem {"OLS from scratch"}</strong> Pick one concrete sub-problem
