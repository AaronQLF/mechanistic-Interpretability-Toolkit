...
export default function QuantChapter() {
  return (
    <ChapterShell
      ...
      chapterSlug="capstone-stats-qa"
      chapters={statisticsChapters}
      eyebrow={"Statistics & Time Series · 09"}
      title={"Capstone: statistics Q&A"}
      lede={"Mixed problems: regression diagnostics, MLE traps, and time-series quickies."}
    >
      <h2>What interviewers want here</h2>
      <p>
        You should be able to explain <strong>{"Capstone: statistics Q&A"}</strong> in plain language,
      ...
      <h2>Core ideas</h2>
      <p>{"Mixed problems: regression diagnostics, MLE traps, and time-series quickies. This chapter stays deliberately dense: skim once for the map, then work the challenge cold before you peek at the solution."}</p>
      ...
      <CodeBlock
        ...
        code={`// capstone stats qa — adapt types and invariants
#include <cstdint>
#include <vector>
...
`}
      ...
      <Challenge
        ...
        prompt={
          <>
            <p>
              <strong>Problem {"Capstone: statistics Q&A"}</strong> Pick one concrete sub-problem
