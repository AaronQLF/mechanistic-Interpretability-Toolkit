...
export default function QuantChapter() {
  return (
    <ChapterShell
      ...
      chapterSlug="ewma-garch"
      chapters={statisticsChapters}
      eyebrow={"Statistics & Time Series · 08"}
      title={"EWMA & GARCH volatility"}
      lede={"RiskMetrics vs GARCH(1,1) — volatility clustering in practice."}
    >
      <h2>What interviewers want here</h2>
      <p>
        You should be able to explain <strong>{"EWMA & GARCH volatility"}</strong> in plain language,
      ...
      <h2>Core ideas</h2>
      <p>{"RiskMetrics vs GARCH(1,1) — volatility clustering in practice. This chapter stays deliberately dense: skim once for the map, then work the challenge cold before you peek at the solution."}</p>
      ...
      <CodeBlock
        ...
        code={`// ewma garch — adapt types and invariants
#include <cstdint>
#include <vector>
...
`}
      ...
      <Challenge
        ...
        prompt={
          <>
            <p>
              <strong>Problem {"EWMA & GARCH volatility"}</strong> Pick one concrete sub-problem
