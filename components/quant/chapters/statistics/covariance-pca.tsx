...
export default function QuantChapter() {
  return (
    <ChapterShell
      ...
      chapterSlug="covariance-pca"
      chapters={statisticsChapters}
      eyebrow={"Statistics & Time Series · 06"}
      title={"Covariance matrices & PCA"}
      lede={"Eigenportfolio thinking without overfitting fantasy."}
    >
      <h2>What interviewers want here</h2>
      <p>
        You should be able to explain <strong>{"Covariance matrices & PCA"}</strong> in plain language,
      ...
      <h2>Core ideas</h2>
      <p>{"Eigenportfolio thinking without overfitting fantasy. This chapter stays deliberately dense: skim once for the map, then work the challenge cold before you peek at the solution."}</p>
      ...
      <CodeBlock
        ...
        code={`// covariance pca — adapt types and invariants
#include <cstdint>
#include <vector>
...
`}
      ...
      <Challenge
        ...
        prompt={
          <>
            <p>
              <strong>Problem {"Covariance matrices & PCA"}</strong> Pick one concrete sub-problem
