...
export default function QuantChapter() {
  return (
    <ChapterShell
      ...
      chapterSlug="mle-moments"
      chapters={statisticsChapters}
      eyebrow={"Statistics & Time Series · 01"}
      title={"MLE & method of moments"}
      lede={"Likelihood, log-likelihood, and when closed forms exist."}
    >
      <h2>What interviewers want here</h2>
      <p>
        You should be able to explain <strong>{"MLE & method of moments"}</strong> in plain language,
      ...
      <h2>Core ideas</h2>
      <p>{"Likelihood, log-likelihood, and when closed forms exist. This chapter stays deliberately dense: skim once for the map, then work the challenge cold before you peek at the solution."}</p>
      ...
      <CodeBlock
        ...
        code={`// mle moments — adapt types and invariants
#include <cstdint>
#include <vector>
...
`}
      ...
      <Challenge
        ...
        prompt={
          <>
            <p>
              <strong>Problem {"MLE & method of moments"}</strong> Pick one concrete sub-problem
