...
export default function QuantChapter() {
  return (
    <ChapterShell
      ...
      chapterSlug="capstone-sql-qa"
      chapters={sqlChapters}
      eyebrow={"SQL & Data Wrangling · 07"}
      title={"Capstone: SQL Q&A"}
      lede={"Multi-part queries under time pressure — window functions mandatory."}
    >
      <h2>What interviewers want here</h2>
      <p>
        You should be able to explain <strong>{"Capstone: SQL Q&A"}</strong> in plain language,
      ...
      <h2>Core ideas</h2>
      <p>{"Multi-part queries under time pressure — window functions mandatory. This chapter stays deliberately dense: skim once for the map, then work the challenge cold before you peek at the solution."}</p>
      ...
      <CodeBlock
        ...
        code={`// capstone sql qa — adapt types and invariants
#include <cstdint>
#include <vector>
...
`}
      ...
      <Challenge
        ...
        prompt={
          <>
            <p>
              <strong>Problem {"Capstone: SQL Q&A"}</strong> Pick one concrete sub-problem
