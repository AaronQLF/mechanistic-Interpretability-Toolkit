...
export default function QuantChapter() {
  return (
    <ChapterShell
      ...
      chapterSlug="time-series-sql"
      chapters={sqlChapters}
      eyebrow={"SQL & Data Wrangling · 05"}
      title={"Time-series patterns in SQL"}
      lede={"As-of joins, gaps and islands, sessionization — interview classics."}
    >
      <h2>What interviewers want here</h2>
      <p>
        You should be able to explain <strong>{"Time-series patterns in SQL"}</strong> in plain language,
      ...
      <h2>Core ideas</h2>
      <p>{"As-of joins, gaps and islands, sessionization — interview classics. This chapter stays deliberately dense: skim once for the map, then work the challenge cold before you peek at the solution."}</p>
      ...
      <CodeBlock
        ...
        code={`// time series sql — adapt types and invariants
#include <cstdint>
#include <vector>
...
`}
      ...
      <Challenge
        ...
        prompt={
          <>
            <p>
              <strong>Problem {"Time-series patterns in SQL"}</strong> Pick one concrete sub-problem
