...
export default function QuantChapter() {
  return (
    <ChapterShell
      ...
      chapterSlug="aggregation-groupby"
      chapters={sqlChapters}
      eyebrow={"SQL & Data Wrangling · 03"}
      title={"Aggregation & GROUP BY"}
      lede={"HAVING vs WHERE, grouping sets — OLAP patterns for ticks."}
    >
      <h2>What interviewers want here</h2>
      <p>
        You should be able to explain <strong>{"Aggregation & GROUP BY"}</strong> in plain language,
      ...
      <h2>Core ideas</h2>
      <p>{"HAVING vs WHERE, grouping sets — OLAP patterns for ticks. This chapter stays deliberately dense: skim once for the map, then work the challenge cold before you peek at the solution."}</p>
      ...
      <CodeBlock
        ...
        code={`// aggregation groupby — adapt types and invariants
#include <cstdint>
#include <vector>
...
`}
      ...
      <Challenge
        ...
        prompt={
          <>
            <p>
              <strong>Problem {"Aggregation & GROUP BY"}</strong> Pick one concrete sub-problem
