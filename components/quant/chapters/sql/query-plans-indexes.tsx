...
export default function QuantChapter() {
  return (
    <ChapterShell
      ...
      chapterSlug="query-plans-indexes"
      chapters={sqlChapters}
      eyebrow={"SQL & Data Wrangling · 06"}
      title={"Query plans & indexes"}
      lede={"EXPLAIN, B-tree vs hash, covering indexes — speak the optimizer's language."}
    >
      <h2>What interviewers want here</h2>
      <p>
        You should be able to explain <strong>{"Query plans & indexes"}</strong> in plain language,
      ...
      <h2>Core ideas</h2>
      <p>{"EXPLAIN, B-tree vs hash, covering indexes — speak the optimizer's language. This chapter stays deliberately dense: skim once for the map, then work the challenge cold before you peek at the solution."}</p>
      ...
      <CodeBlock
        ...
        code={`// query plans indexes — adapt types and invariants
#include <cstdint>
#include <vector>
...
`}
      ...
      <Challenge
        ...
        prompt={
          <>
            <p>
              <strong>Problem {"Query plans & indexes"}</strong> Pick one concrete sub-problem
