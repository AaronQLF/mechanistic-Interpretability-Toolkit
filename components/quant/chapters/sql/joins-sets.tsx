...
export default function QuantChapter() {
  return (
    <ChapterShell
      ...
      chapterSlug="joins-sets"
      chapters={sqlChapters}
      eyebrow={"SQL & Data Wrangling · 02"}
      title={"Joins & set operations"}
      lede={"INNER/LEFT/CROSS, UNION vs UNION ALL — know the Venn diagrams cold."}
    >
      <h2>What interviewers want here</h2>
      <p>
        You should be able to explain <strong>{"Joins & set operations"}</strong> in plain language,
      ...
      <h2>Core ideas</h2>
      <p>{"INNER/LEFT/CROSS, UNION vs UNION ALL — know the Venn diagrams cold. This chapter stays deliberately dense: skim once for the map, then work the challenge cold before you peek at the solution."}</p>
      ...
      <CodeBlock
        ...
        code={`// joins sets — adapt types and invariants
#include <cstdint>
#include <vector>
...
`}
      ...
      <Challenge
        ...
        prompt={
          <>
            <p>
              <strong>Problem {"Joins & set operations"}</strong> Pick one concrete sub-problem
