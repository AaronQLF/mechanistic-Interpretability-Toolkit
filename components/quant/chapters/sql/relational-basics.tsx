...
import { sqlChapters } from "@/lib/quant";

export default function QuantChapter() {
  return (
    <ChapterShell
      moduleSlug={`quant/sql`}
      chapterSlug="relational-basics"
      chapters={sqlChapters}
      eyebrow={"SQL & Data Wrangling · 01"}
      title={"Relational model & SQL basics"}
      lede={"Keys, constraints, normalization — the theory interviewers still reference."}
    >
      <h2>What interviewers want here</h2>
      <p>
        You should be able to explain <strong>{"Relational model & SQL basics"}</strong> in plain language,
      ...
      <h2>Core ideas</h2>
      <p>{"Keys, constraints, normalization — the theory interviewers still reference. This chapter stays deliberately dense: skim once for the map, then work the challenge cold before you peek at the solution."}</p>
      ...
      <CodeBlock
        ...
        code={`// relational basics — adapt types and invariants
#include <cstdint>
#include <vector>
...
`}
      ...
      <Challenge
        ...
        prompt={
          <>
            <p>
              <strong>Problem {"Relational model & SQL basics"}</strong> Pick one concrete sub-problem
