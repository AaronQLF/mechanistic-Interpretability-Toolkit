...
export default function QuantChapter() {
  return (
    <ChapterShell
      ...
      chapterSlug="window-functions"
      chapters={sqlChapters}
      eyebrow={"SQL & Data Wrangling · 04"}
      title={"Window functions"}
      lede={"PARTITION BY, ROWS vs RANGE, running sums — the Swiss Army knife."}
    >
      <h2>What interviewers want here</h2>
      <p>
        You should be able to explain <strong>{"Window functions"}</strong> in plain language,
      ...
      <h2>Core ideas</h2>
      <p>{"PARTITION BY, ROWS vs RANGE, running sums — the Swiss Army knife. This chapter stays deliberately dense: skim once for the map, then work the challenge cold before you peek at the solution."}</p>
      ...
      <CodeBlock
        ...
        code={`// window functions — adapt types and invariants
#include <cstdint>
#include <vector>
...
`}
      ...
      <Challenge
        ...
        prompt={
          <>
            <p>
              <strong>Problem {"Window functions"}</strong> Pick one concrete sub-problem
