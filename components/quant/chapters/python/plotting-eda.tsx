...
export default function QuantChapter() {
  return (
    <ChapterShell
      ...
      chapterSlug="plotting-eda"
      chapters={pythonChapters}
      eyebrow={"Python & Tooling · 06"}
      title={"Plotting & exploratory analysis"}
      lede={"matplotlib/seaborn discipline — readable plots under time pressure."}
    >
      <h2>What interviewers want here</h2>
      <p>
        You should be able to explain <strong>{"Plotting & exploratory analysis"}</strong> in plain language,
      ...
      <h2>Core ideas</h2>
      <p>{"matplotlib/seaborn discipline — readable plots under time pressure. This chapter stays deliberately dense: skim once for the map, then work the challenge cold before you peek at the solution."}</p>
      ...
      <CodeBlock
        ...
        code={`// plotting eda — adapt types and invariants
#include <cstdint>
#include <vector>
...
`}
      ...
      <Challenge
        ...
        prompt={
          <>
            <p>
              <strong>Problem {"Plotting & exploratory analysis"}</strong> Pick one concrete sub-problem
