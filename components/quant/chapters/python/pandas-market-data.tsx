...
export default function QuantChapter() {
  return (
    <ChapterShell
      ...
      chapterSlug="pandas-market-data"
      chapters={pythonChapters}
      eyebrow={"Python & Tooling · 03"}
      title={"pandas for market data"}
      lede={"Time indexes, resampling, rolling windows, and merge_asof for as-of joins."}
    >
      <h2>What interviewers want here</h2>
      <p>
        You should be able to explain <strong>{"pandas for market data"}</strong> in plain language,
      ...
      <h2>Core ideas</h2>
      <p>{"Time indexes, resampling, rolling windows, and merge_asof for as-of joins. This chapter stays deliberately dense: skim once for the map, then work the challenge cold before you peek at the solution."}</p>
      ...
      <CodeBlock
        ...
        code={`// pandas market data — adapt types and invariants
#include <cstdint>
#include <vector>
...
`}
      ...
      <Challenge
        ...
        prompt={
          <>
            <p>
              <strong>Problem {"pandas for market data"}</strong> Pick one concrete sub-problem
