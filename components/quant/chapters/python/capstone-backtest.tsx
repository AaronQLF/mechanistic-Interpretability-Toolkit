...
export default function QuantChapter() {
  return (
    <ChapterShell
      ...
      chapterSlug="capstone-backtest"
      chapters={pythonChapters}
      eyebrow={"Python & Tooling · 07"}
      title={"Capstone: build a minimal backtester"}
      lede={"Event loop, positions, and metrics — no look-ahead, no fantasy fills."}
    >
      <h2>What interviewers want here</h2>
      <p>
        You should be able to explain <strong>{"Capstone: build a minimal backtester"}</strong> in plain language,
      ...
      <h2>Core ideas</h2>
      <p>{"Event loop, positions, and metrics — no look-ahead, no fantasy fills. This chapter stays deliberately dense: skim once for the map, then work the challenge cold before you peek at the solution."}</p>
      ...
      <CodeBlock
        ...
        code={`// capstone backtest — adapt types and invariants
#include <cstdint>
#include <vector>
...
`}
      ...
      <Challenge
        ...
        prompt={
          <>
            <p>
              <strong>Problem {"Capstone: build a minimal backtester"}</strong> Pick one concrete sub-problem
