...
export default function QuantChapter() {
  return (
    <ChapterShell
      ...
      chapterSlug="gil-profiling-numba"
      chapters={pythonChapters}
      eyebrow={"Python & Tooling · 04"}
      title={"GIL, profiling & Numba"}
      lede={"When threads lie, cProfile, and JIT for inner loops."}
    >
      <h2>What interviewers want here</h2>
      <p>
        You should be able to explain <strong>{"GIL, profiling & Numba"}</strong> in plain language,
      ...
      <h2>Core ideas</h2>
      <p>{"When threads lie, cProfile, and JIT for inner loops. This chapter stays deliberately dense: skim once for the map, then work the challenge cold before you peek at the solution."}</p>
      ...
      <CodeBlock
        ...
        code={`// gil profiling numba — adapt types and invariants
#include <cstdint>
#include <vector>
...
`}
      ...
      <Challenge
        ...
        prompt={
          <>
            <p>
              <strong>Problem {"GIL, profiling & Numba"}</strong> Pick one concrete sub-problem
