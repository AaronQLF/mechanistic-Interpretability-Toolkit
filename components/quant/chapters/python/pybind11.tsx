...
export default function QuantChapter() {
  return (
    <ChapterShell
      ...
      chapterSlug="pybind11"
      chapters={pythonChapters}
      eyebrow={"Python & Tooling · 05"}
      title={"pybind11: Python meets C++"}
      lede={"Expose hot C++ to Python safely — the quant research stack pattern."}
    >
      <h2>What interviewers want here</h2>
      <p>
        You should be able to explain <strong>{"pybind11: Python meets C++"}</strong> in plain language,
      ...
      <h2>Core ideas</h2>
      <p>{"Expose hot C++ to Python safely — the quant research stack pattern. This chapter stays deliberately dense: skim once for the map, then work the challenge cold before you peek at the solution."}</p>
      ...
      <CodeBlock
        ...
        code={`// pybind11 — adapt types and invariants
#include <cstdint>
#include <vector>
...
`}
      ...
      <Challenge
        ...
        prompt={
          <>
            <p>
              <strong>Problem {"pybind11: Python meets C++"}</strong> Pick one concrete sub-problem
