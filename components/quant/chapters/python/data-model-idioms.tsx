...
import { pythonChapters } from "@/lib/quant";

export default function QuantChapter() {
  return (
    <ChapterShell
      moduleSlug={`quant/python`}
      chapterSlug="data-model-idioms"
      chapters={pythonChapters}
      eyebrow={"Python & Tooling · 01"}
      title={"Python data model & idioms"}
      lede={"Objects, references, mutability, dunder methods — write Python that scales to notebooks and prod."}
    >
      <h2>What interviewers want here</h2>
      <p>
        You should be able to explain <strong>{"Python data model & idioms"}</strong> in plain language,
      ...
      <h2>Core ideas</h2>
      <p>{"Objects, references, mutability, dunder methods — write Python that scales to notebooks and prod. This chapter stays deliberately dense: skim once for the map, then work the challenge cold before you peek at the solution."}</p>
      ...
      <CodeBlock
        ...
        code={`// data model idioms — adapt types and invariants
#include <cstdint>
#include <vector>
...
`}
      ...
      <Challenge
        ...
        prompt={
          <>
            <p>
              <strong>Problem {"Python data model & idioms"}</strong> Pick one concrete sub-problem
