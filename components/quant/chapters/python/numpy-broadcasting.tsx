...
export default function QuantChapter() {
  return (
    <ChapterShell
      ...
      chapterSlug="numpy-broadcasting"
      chapters={pythonChapters}
      eyebrow={"Python & Tooling · 02"}
      title={"NumPy & broadcasting"}
      lede={"Vectorization, strides, and avoiding accidental Python loops."}
    >
      <h2>What interviewers want here</h2>
      <p>
        You should be able to explain <strong>{"NumPy & broadcasting"}</strong> in plain language,
      ...
      <h2>Core ideas</h2>
      <p>{"Vectorization, strides, and avoiding accidental Python loops. This chapter stays deliberately dense: skim once for the map, then work the challenge cold before you peek at the solution."}</p>
      ...
      <CodeBlock
        ...
        code={`// numpy broadcasting — adapt types and invariants
#include <cstdint>
#include <vector>
...
`}
      ...
      <Challenge
        ...
        prompt={
          <>
            <p>
              <strong>Problem {"NumPy & broadcasting"}</strong> Pick one concrete sub-problem
