...
export default function QuantChapter() {
  return (
    <ChapterShell
      ...
      chapterSlug="allocation-numa-hugepages"
      chapters={systemsChapters}
      eyebrow={"Low-Latency Systems · 06"}
      title={"Allocation, NUMA & huge pages"}
      lede={"Arena allocators, lock-free pools, numactl, and transparent huge pages caveats."}
    >
      <h2>What interviewers want here</h2>
      <p>
        You should be able to explain <strong>{"Allocation, NUMA & huge pages"}</strong> in plain language,
      ...
      <h2>Core ideas</h2>
      <p>{"Arena allocators, lock-free pools, numactl, and transparent huge pages caveats. This chapter stays deliberately dense: skim once for the map, then work the challenge cold before you peek at the solution."}</p>
      ...
      <CodeBlock
        ...
        code={`// allocation numa hugepages — adapt types and invariants
#include <cstdint>
#include <vector>
...
`}
      ...
      <Challenge
        ...
        prompt={
          <>
            <p>
              <strong>Problem {"Allocation, NUMA & huge pages"}</strong> Pick one concrete sub-problem
