...
export default function QuantChapter() {
  return (
    <ChapterShell
      ...
      chapterSlug="kernel-bypass-polling"
      chapters={systemsChapters}
      eyebrow={"Low-Latency Systems · 07"}
      title={"Kernel bypass & busy polling"}
      lede={"DPDK, onload, busy spin vs interrupt — when the kernel is too slow."}
    >
      <h2>What interviewers want here</h2>
      <p>
        You should be able to explain <strong>{"Kernel bypass & busy polling"}</strong> in plain language,
      ...
      <h2>Core ideas</h2>
      <p>{"DPDK, onload, busy spin vs interrupt — when the kernel is too slow. This chapter stays deliberately dense: skim once for the map, then work the challenge cold before you peek at the solution."}</p>
      ...
      <CodeBlock
        ...
        code={`// kernel bypass polling — adapt types and invariants
#include <cstdint>
#include <vector>
...
`}
      ...
      <Challenge
        ...
        prompt={
          <>
            <p>
              <strong>Problem {"Kernel bypass & busy polling"}</strong> Pick one concrete sub-problem
