...
export default function QuantChapter() {
  return (
    <ChapterShell
      ...
      chapterSlug="capstone-hot-path"
      chapters={systemsChapters}
      eyebrow={"Low-Latency Systems · 09"}
      title={"Capstone: optimize a hot path"}
      lede={"Take a naive order-book update loop and squeeze real microseconds out of it."}
    >
      <h2>What interviewers want here</h2>
      <p>
        You should be able to explain <strong>{"Capstone: optimize a hot path"}</strong> in plain language,
      ...
      <h2>Core ideas</h2>
      <p>{"Take a naive order-book update loop and squeeze real microseconds out of it. This chapter stays deliberately dense: skim once for the map, then work the challenge cold before you peek at the solution."}</p>
      ...
      <CodeBlock
        ...
        code={`// capstone hot path — adapt types and invariants
#include <cstdint>
#include <vector>
...
`}
      ...
      <Challenge
        ...
        prompt={
          <>
            <p>
              <strong>Problem {"Capstone: optimize a hot path"}</strong> Pick one concrete sub-problem
