import { LatencyNumbersChart } from "@/components/viz/quant/LatencyNumbersChart";
...
import { systemsChapters } from "@/lib/quant";

export default function QuantChapter() {
  return (
    <ChapterShell
      moduleSlug={`quant/systems`}
      chapterSlug="latency-hierarchy"
      chapters={systemsChapters}
      eyebrow={"Low-Latency Systems · 01"}
      title={"The latency hierarchy"}
      lede={"Numbers every quant dev memorizes: L1, RAM, SSD, network — orders of magnitude."}
    >
      <h2>What interviewers want here</h2>
      <p>
        You should be able to explain <strong>{"The latency hierarchy"}</strong> in plain language,
      ...
      <h2>Core ideas</h2>
      <p>{"Numbers every quant dev memorizes: L1, RAM, SSD, network — orders of magnitude. This chapter stays deliberately dense: skim once for the map, then work the challenge cold before you peek at the solution."}</p>
      ...
      <CodeBlock
        ...
        code={`// latency hierarchy — adapt types and invariants
#include <cstdint>
#include <vector>
...
`}
      ...
      <Challenge
        ...
        prompt={
          <>
            <p>
              <strong>Problem {"The latency hierarchy"}</strong> Pick one concrete sub-problem
