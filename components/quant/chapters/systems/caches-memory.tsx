...
export default function QuantChapter() {
  return (
    <ChapterShell
      ...
      chapterSlug="caches-memory"
      chapters={systemsChapters}
      eyebrow={"Low-Latency Systems · 02"}
      title={"Caches & memory hierarchy"}
      lede={"Lines, associativity, prefetching — why layout matters more than micro-ops."}
    >
      <h2>What interviewers want here</h2>
      <p>
        You should be able to explain <strong>{"Caches & memory hierarchy"}</strong> in plain language,
      ...
      <h2>Core ideas</h2>
      <p>{"Lines, associativity, prefetching — why layout matters more than micro-ops. This chapter stays deliberately dense: skim once for the map, then work the challenge cold before you peek at the solution."}</p>
      ...
      <CodeBlock
        ...
        code={`// caches memory — adapt types and invariants
#include <cstdint>
#include <vector>
...
`}
      ...
      <Challenge
        ...
        prompt={
          <>
            <p>
              <strong>Problem {"Caches & memory hierarchy"}</strong> Pick one concrete sub-problem
