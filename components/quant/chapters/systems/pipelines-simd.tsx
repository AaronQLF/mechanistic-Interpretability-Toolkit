...
export default function QuantChapter() {
  return (
    <ChapterShell
      ...
      chapterSlug="pipelines-simd"
      chapters={systemsChapters}
      eyebrow={"Low-Latency Systems · 04"}
      title={"Pipelines, ILP & SIMD"}
      lede={"Superscalar execution, auto-vectorization, and explicit intrinsics trade-offs."}
    >
      <h2>What interviewers want here</h2>
      <p>
        You should be able to explain <strong>{"Pipelines, ILP & SIMD"}</strong> in plain language,
      ...
      <h2>Core ideas</h2>
      <p>{"Superscalar execution, auto-vectorization, and explicit intrinsics trade-offs. This chapter stays deliberately dense: skim once for the map, then work the challenge cold before you peek at the solution."}</p>
      ...
      <CodeBlock
        ...
        code={`// pipelines simd — adapt types and invariants
#include <cstdint>
#include <vector>
...
`}
      ...
      <Challenge
        ...
        prompt={
          <>
            <p>
              <strong>Problem {"Pipelines, ILP & SIMD"}</strong> Pick one concrete sub-problem
