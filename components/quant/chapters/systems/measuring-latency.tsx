...
export default function QuantChapter() {
  return (
    <ChapterShell
      ...
      chapterSlug="measuring-latency"
      chapters={systemsChapters}
      eyebrow={"Low-Latency Systems · 08"}
      title={"Measuring latency & jitter"}
      lede={"Histograms, percentiles, coordinated omission — how not to lie with benchmarks."}
    >
      <h2>What interviewers want here</h2>
      <p>
        You should be able to explain <strong>{"Measuring latency & jitter"}</strong> in plain language,
      ...
      <h2>Core ideas</h2>
      <p>{"Histograms, percentiles, coordinated omission — how not to lie with benchmarks. This chapter stays deliberately dense: skim once for the map, then work the challenge cold before you peek at the solution."}</p>
      ...
      <CodeBlock
        ...
        code={`// measuring latency — adapt types and invariants
#include <cstdint>
#include <vector>
...
`}
      ...
      <Challenge
        ...
        prompt={
          <>
            <p>
              <strong>Problem {"Measuring latency & jitter"}</strong> Pick one concrete sub-problem
