...
export default function QuantChapter() {
  return (
    <ChapterShell
      ...
      chapterSlug="combinatorial-puzzles"
      chapters={brainteasersChapters}
      eyebrow={"Brainteasers & Games · 03"}
      title={"Combinatorial puzzles"}
      lede={"Invariants, coloring arguments, and double counting."}
    >
      <h2>What interviewers want here</h2>
      <p>
        You should be able to explain <strong>{"Combinatorial puzzles"}</strong> in plain language,
      ...
      <h2>Core ideas</h2>
      <p>{"Invariants, coloring arguments, and double counting. This chapter stays deliberately dense: skim once for the map, then work the challenge cold before you peek at the solution."}</p>
      ...
      <CodeBlock
        ...
        code={`// combinatorial puzzles — adapt types and invariants
#include <cstdint>
#include <vector>
...
`}
      ...
      <Challenge
        ...
        prompt={
          <>
            <p>
              <strong>Problem {"Combinatorial puzzles"}</strong> Pick one concrete sub-problem
