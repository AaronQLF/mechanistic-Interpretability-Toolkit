...
export default function QuantChapter() {
  return (
    <ChapterShell
      ...
      chapterSlug="fermi-estimation"
      chapters={brainteasersChapters}
      eyebrow={"Brainteasers & Games · 06"}
      title={"Fermi estimation"}
      lede={"Sanity checks, geometric means, and communicating uncertainty."}
    >
      <h2>What interviewers want here</h2>
      <p>
        You should be able to explain <strong>{"Fermi estimation"}</strong> in plain language,
      ...
      <h2>Core ideas</h2>
      <p>{"Sanity checks, geometric means, and communicating uncertainty. This chapter stays deliberately dense: skim once for the map, then work the challenge cold before you peek at the solution."}</p>
      ...
      <CodeBlock
        ...
        code={`// fermi estimation — adapt types and invariants
#include <cstdint>
#include <vector>
...
`}
      ...
      <Challenge
        ...
        prompt={
          <>
            <p>
              <strong>Problem {"Fermi estimation"}</strong> Pick one concrete sub-problem
