...
export default function QuantChapter() {
  return (
    <ChapterShell
      ...
      chapterSlug="game-theory"
      chapters={brainteasersChapters}
      eyebrow={"Brainteasers & Games · 08"}
      title={"Game theory basics"}
      lede={"Nash equilibrium, dominant strategies, backward induction on trees."}
    >
      <h2>What interviewers want here</h2>
      <p>
        You should be able to explain <strong>{"Game theory basics"}</strong> in plain language,
      ...
      <h2>Core ideas</h2>
      <p>{"Nash equilibrium, dominant strategies, backward induction on trees. This chapter stays deliberately dense: skim once for the map, then work the challenge cold before you peek at the solution."}</p>
      ...
      <CodeBlock
        ...
        code={`// game theory — adapt types and invariants
#include <cstdint>
#include <vector>
...
`}
      ...
      <Challenge
        ...
        prompt={
          <>
            <p>
              <strong>Problem {"Game theory basics"}</strong> Pick one concrete sub-problem
