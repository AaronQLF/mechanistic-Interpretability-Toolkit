...
export default function QuantChapter() {
  return (
    <ChapterShell
      ...
      chapterSlug="ev-puzzles"
      chapters={brainteasersChapters}
      eyebrow={"Brainteasers & Games · 04"}
      title={"Expected value puzzles"}
      lede={"Coupon collector, records, stopping rules — when to recurse on states."}
    >
      <h2>What interviewers want here</h2>
      <p>
        You should be able to explain <strong>{"Expected value puzzles"}</strong> in plain language,
      ...
      <h2>Core ideas</h2>
      <p>{"Coupon collector, records, stopping rules — when to recurse on states. This chapter stays deliberately dense: skim once for the map, then work the challenge cold before you peek at the solution."}</p>
      ...
      <CodeBlock
        ...
        code={`// ev puzzles — adapt types and invariants
#include <cstdint>
#include <vector>
...
`}
      ...
      <Challenge
        ...
        prompt={
          <>
            <p>
              <strong>Problem {"Expected value puzzles"}</strong> Pick one concrete sub-problem
