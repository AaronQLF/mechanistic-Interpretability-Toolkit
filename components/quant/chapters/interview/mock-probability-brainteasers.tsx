...
export default function QuantChapter() {
  return (
    <ChapterShell
      ...
      chapterSlug="mock-probability-brainteasers"
      chapters={interviewChapters}
      eyebrow={"Interview Strategy & Mocks · 07"}
      title={"Mock set: probability & brainteasers"}
      lede={"Short problems with full solutions — speed and clarity."}
    >
      <h2>What interviewers want here</h2>
      <p>
        You should be able to explain <strong>{"Mock set: probability & brainteasers"}</strong> in plain language,
      ...
      <h2>Core ideas</h2>
      <p>{"Short problems with full solutions — speed and clarity. This chapter stays deliberately dense: skim once for the map, then work the challenge cold before you peek at the solution."}</p>
      ...
      <CodeBlock
        ...
        code={`// mock probability brainteasers — adapt types and invariants
#include <cstdint>
#include <vector>
...
`}
      ...
      <Challenge
        ...
        prompt={
          <>
            <p>
              <strong>Problem {"Mock set: probability & brainteasers"}</strong> Pick one concrete sub-problem
