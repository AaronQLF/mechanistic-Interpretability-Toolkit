...
export default function QuantChapter() {
  return (
    <ChapterShell
      ...
      chapterSlug="mock-coding-dsa"
      chapters={interviewChapters}
      eyebrow={"Interview Strategy & Mocks · 06"}
      title={"Mock set: coding & DSA"}
      lede={"A timed set mixing arrays, graphs, and implementation detail."}
    >
      <h2>What interviewers want here</h2>
      <p>
        You should be able to explain <strong>{"Mock set: coding & DSA"}</strong> in plain language,
      ...
      <h2>Core ideas</h2>
      <p>{"A timed set mixing arrays, graphs, and implementation detail. This chapter stays deliberately dense: skim once for the map, then work the challenge cold before you peek at the solution."}</p>
      ...
      <CodeBlock
        ...
        code={`// mock coding dsa — adapt types and invariants
#include <cstdint>
#include <vector>
...
`}
      ...
      <Challenge
        ...
        prompt={
          <>
            <p>
              <strong>Problem {"Mock set: coding & DSA"}</strong> Pick one concrete sub-problem
