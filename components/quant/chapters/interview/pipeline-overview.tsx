...
export default function QuantChapter() {
  return (
    <ChapterShell
      ...
      chapterSlug="pipeline-overview"
      chapters={interviewChapters}
      eyebrow={"Interview Strategy & Mocks · 01"}
      title={"The quant developer interview pipeline"}
      lede={"Phone screens, onsite loops, take-homes — what each stage optimizes for."}
    >
      <h2>What interviewers want here</h2>
      <p>
        You should be able to explain <strong>{"The quant developer interview pipeline"}</strong> in plain language,
      ...
      <h2>Core ideas</h2>
      <p>{"Phone screens, onsite loops, take-homes — what each stage optimizes for. This chapter stays deliberately dense: skim once for the map, then work the challenge cold before you peek at the solution."}</p>
      ...
      <CodeBlock
        ...
        code={`// pipeline overview — adapt types and invariants
#include <cstdint>
#include <vector>
...
`}
      ...
      <Challenge
        ...
        prompt={
          <>
            <p>
              <strong>Problem {"The quant developer interview pipeline"}</strong> Pick one concrete sub-problem
