...
export default function QuantChapter() {
  return (
    <ChapterShell
      ...
      chapterSlug="mock-pricing-math"
      chapters={interviewChapters}
      eyebrow={"Interview Strategy & Mocks · 09"}
      title={"Mock set: pricing & stochastic math"}
      lede={"Derive, discretize, implement — connect math to code."}
    >
      <h2>What interviewers want here</h2>
      <p>
        You should be able to explain <strong>{"Mock set: pricing & stochastic math"}</strong> in plain language,
      ...
      <h2>Core ideas</h2>
      <p>{"Derive, discretize, implement — connect math to code. This chapter stays deliberately dense: skim once for the map, then work the challenge cold before you peek at the solution."}</p>
      ...
      <CodeBlock
        ...
        code={`// mock pricing math — adapt types and invariants
#include <cstdint>
#include <vector>
...
`}
      ...
      <Challenge
        ...
        prompt={
          <>
            <p>
              <strong>Problem {"Mock set: pricing & stochastic math"}</strong> Pick one concrete sub-problem
